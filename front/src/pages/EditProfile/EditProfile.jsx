...
function EditProfile(props) {
    ...
    useEffect(() => {
        const getProfile = async() => {
            try {
                const response = await axios.get("http://localhost:8080/servlet_study/mypage/profile", {
                    headers: {
    ...
    const handleUpdateClick = () => {
        ...
        const submit = async () => {
            ...
            const response =  await axios.put("http://localhost:8080/servlet_study/mypage/profile", profile, option);
            if(response.data) {
                alert("수정 완료.!!");
                navigate("/mypage");
                return;
